...
describe('Own checks', function() {
		it('expect submitHomework with valid IDs to not throw', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting']);
		...
		it('expect submitHomework with valid IDs [one presentation:four students] to not throw', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting']);
			course.addStudent('John Doe');
			course.addStudent('John Second');
			course.addStudent('John Third');
			course.addStudent('John Fourth');
			course.submitHomework(1,1);
			course.submitHomework(2,1);
			course.submitHomework(3,1);
			course.submitHomework(4,1);
			}
			expect(test).to.not.throw();
		});

		it('expect submitHomework with valid IDs [three presentation:four students] to not throw', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting', 'MonkeyBiology', 'Algorithms']);
			course.addStudent('John Doe');
			course.addStudent('John Second');
			course.addStudent('John Third');
			course.addStudent('John Fourth');
			// submit only homeworks for the last presentation because of the taks discription 
			//to trow if homeworkId is bigger than presentation list
			course.submitHomework(1,3);
			course.submitHomework(2,3);
			course.submitHomework(3,3);
			course.submitHomework(4,3);
			}
			expect(test).to.not.throw();
		});

		it('expect pushExamResults with invalid IDs to throw', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting']);
			course.addStudent('John Doe');
			course.submitHomework(1,1);
			course.pushExamResults([{StudentID : 2, Score: 50}]);
			}
			expect(test).to.throw();
		});
		it('expect pushExamResults with valid IDs not to throw', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting']);
			course.addStudent('John Doe');
			course.submitHomework(1,1);
			course.pushExamResults([{StudentID : 1, Score: 50}]);
			}
			expect(test).to.not.throw();
		});
		it('expect pushExamResults with duplicate StudentIDs to throw', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting']);
			course.addStudent('John Doe');
			...
			course.submitHomework(2,1);
			course.submitHomework(2,1);
			course.pushExamResults([{StudentID : 1, Score: 50},{StudentID : 2, Score: 150},{StudentID : 1, Score: 300}]);
			}
			expect(test).to.throw();
		});
		it('expect pushExamResults with valid StudentIDs and scores Not to throw', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting']);
			course.addStudent('John Doe');
			...
			course.submitHomework(2,1);
			course.submitHomework(3,1);
			course.pushExamResults([{StudentID : 1, Score: 50},{StudentID : 2, Score: 150},{StudentID : 3, Score: 300}]);
			}
			expect(test).to.not.throw();
		});
		it('expect getTopStudents with valid input to return array with top students', function(){
			function test(){
				var course = Object.create(Course);
			course.init('TestJS', ['MochaTesting']);
			course.addStudent('John Doe');
			course.addStudent('John Second');
			course.addStudent('John Third');
			course.addStudent('John Forth');
			course.addStudent('John Fifth');
			course.addStudent('John Sixth');
			course.submitHomework(1,1);
			course.submitHomework(2,1);
			course.submitHomework(3,1);
			course.pushExamResults([{StudentID : 1, Score: 50},{StudentID : 2, Score: 150},
				{StudentID : 3, Score: 300},{StudentID : 4, Score: 225},{StudentID : 5, Score: 172},{StudentID : 6, Score: 100}]);
